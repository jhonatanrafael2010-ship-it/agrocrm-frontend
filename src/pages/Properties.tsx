...
const PropertiesTalhoesPage: React.FC = () => {
  ...
  async function createProperty() {
    ...
    try {
      ...
      alert(err?.message || 'Erro ao criar propriedade')
                ...
                {loading ? (
                  ...
                  properties.map(p => (
                    <tr key={p.id}>
                      ...
                      <td>{p.area_ha ?? '--'}</td>
                      <td>
                        <button className="btn-action btn-delete" onClick={() => deleteProperty(p.id)} title="Excluir">
                          <img src={trashIcon} alt="Excluir" style={{ width: 20, height: 20, verticalAlign: 'middle' }} />
                        </button>
                      </td>
                    </tr>
                  ))
          ...
          <div className="modal-overlay" role="dialog" aria-modal="true">
            <div className="modal">
              ...
              <div className="form-row">
                <label>Cliente</label>
                <DarkSelect name="client_id" value={propForm.client_id} placeholder="Selecione um cliente" options={[{ value: '', label: 'Selecione um cliente' }, ...clients.map(c => ({ value: String(c.id), label: c.name }))]} onChange={handlePropChange as any} />
              </div>
              ...
              <div className="modal-actions"><button className="btn-cancel" onClick={() => setOpenProp(false)}>Cancelar</button><button className="btn-save" onClick={createProperty} disabled={submitting}>{submitting ? 'Salvando...' : 'Salvar'}</button></div>
            </div>
          </div>
        )}
        {openPlot && (
